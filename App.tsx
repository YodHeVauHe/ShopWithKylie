...
import { INITIAL_PRODUCTS, SALES_DATA, CATEGORY_DATA } from './constants';
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <div className="hidden lg:flex justify-end p-4 lg:p-6 sticky top-0 z-20">
           <button 
             ...
             className="px-4 py-2 rounded-full glass-button text-xs font-bold text-neutral-300 hover:text-white flex items-center gap-2 uppercase tracking-wide"
           >
             View Storefront
...
const ToastContainer = ({ toasts }: { toasts: ToastMessage[] }) => (
  <div className="fixed bottom-6 right-6 z-[70] flex flex-col gap-3 pointer-events-none">
    {toasts.map(toast => (
      <div 
        key={toast.id}
        className={`pointer-events-auto flex items-center w-full max-w-xs p-4 rounded-xl shadow-2xl text-white transform transition-all duration-300 ease-in-out translate-y-0 opacity-100 backdrop-blur-xl border border-white/10 ${
